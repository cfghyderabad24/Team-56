...
function ResponsiveAppBar({ store }) {
  ...
  const handleCloseUserMenu = (event) => {
    ...
    // Dispatch actions based on the selected setting
    if (selectedSetting === 'Entry') {
      store.dispatch({ type: 'Entry' });
    } else if (selectedSetting === 'Profile') {
      store.dispatch({ type: 'Profile' });
    } else if (selectedSetting === 'Detail') {
      store.dispatch({ type: 'Detail' });
    } else if (selectedSetting === 'Logout') {
      store.dispatch({ type: 'logout' });
      // After logout, display success message
      Swal.fire({
